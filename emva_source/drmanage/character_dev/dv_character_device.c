...
EmvaLink_List *g_characterModuleDeviceList = NULL;
...
int32 character_dev_open(const char *device_name, void *obj)
{
	...
	{
		character_driver_t *p = (character_driver_t *)EmvaLink_List_GetAt(g_characterModuleDeviceList, i);
		if (strcmp(p->DeviceName, device_name) == 0)
		{
			if(p->ops->open_module(obj)!=-1)
			{
				return i;
			}
			return -1;
...
emva_error *character_dev_device_registered(character_driver_t *ModuleDevice)
{
	...
	if (g_characterModuleDeviceList == NULL)
	{
		g_characterModuleDeviceList = EmvaLink_List_Init();
	}
	if (g_characterModuleDeviceList->length == 0)
	{
		EmvaLink_List_Insert(g_characterModuleDeviceList, ModuleDevice, -1);
		return NULL;
	}
	else
	{
	...
	{
		emva_free(g_characterModuleDevArray);
	}
	g_characterModuleDevArray = (character_driver_t **)emva_malloc(sizeof(character_driver_t *) * g_characterModuleDeviceList->length);
...
EMVA_BOOL character_dev_device_remove(const char *name)
{
	for (uint8 i = 0; i < g_characterModuleDeviceList->length; i++)
	{
		...
		{
			EmvaLink_List_RemoveAt(g_characterModuleDeviceList, i);
			return TRUE;
		}
	}
	return FALSE;
