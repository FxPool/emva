...
**/
static emva_error *ConnEvent(EmvaConn *this, void (*p)(void))
{
  event_callback = p;
  return NULL;
}
/**
  ...
  * @retval : Error description
**/
static emva_error *ConnRead(EmvaConn *this, byte *b, uint32 nread, int32 *nrecv)
{
  ...
  do
  {
    *nrecv = emva_kfifo_get(this->fifo, b, nread);
    emva_sleep(1);
  } while (*nrecv == 0);
  return NULL;
}
/**
  ...
  * @retval :   Error description
**/
static emva_error *ConnWrite(EmvaConn *this, byte *b, uint32 n)
{
...
/**
  ...
  * @retval :   Error description
**/
static emva_error *ConnClose(EmvaConn *this)
{
...
/**
  ...
  * @retval :   Error description
**/
static emva_error *ConnSetDeadline(EmvaConn *this, uint32 t)
{
...
/**
  ...
  * @retval :   Error description
**/
static emva_error *ConnSetReadDeadline(EmvaConn *this, uint32 t)
{
  this->err->new (this->err, "no over write");
  return this->err;
}
/**
  ...
  * @retval :   Error description
**/
static emva_error *ConnSetWriteDeadline(EmvaConn *this, uint32 t)
{
  this->err->new (this->err, "no over write");
  return this->err;
}
...
/**
  * @brief  	ThisConnectNewAddress
  * @param  	ThisConnectNewAddress. ThisConnectNewAddress
  * @retval : bool
**/
static EmvaConn * ThisConnectNewAddress(EmvaConn *this,emva_error *err, char *Network, char *address)
{
	uint8 netState = 0;
  ...
  if (net_dev_do_ioctl(g_fd, this, MAGIC_SOCKET, (uint32)&sck) == -1)
  {
    if (err)
  ...
  {
    net_dev_event(g_fd, this, data_callback);
  }
  if (err)
    err->err_info = NULL;
	return this;
...
**/
static EmvaConn *ThisDial(emva_error *err, char *Network, char *address)
{
  uint8 netState = 0;
  if (net_dev_do_ioctl(g_fd, &netState, MAGIC_IS_NET_CONNECT, NULL) == -1)
  {
  ...
  conn->Close = ConnClose;
	conn->ConnectNewAddress = ThisConnectNewAddress;
  ...
  conn->err = emva_error_new(128);
  conn->fifo = (struct emva_kfifo *)emva_malloc(sizeof(struct emva_kfifo));
  emva_kfifo_init(conn->fifo, kfifo_buffer, sizeof(kfifo_buffer));
  /* If the hardware supports multiple sockets */
  struct socket
  {
    char *network;
