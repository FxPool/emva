...
EMVA_BOOL timerDel(EmvaTime *p)
{
    ...
    Link_List_object_RemoveAt(g_timeInstancelist, &obj);
    if (p)
    {
        emva_free(p);
    }
...
inline void virtualTimerRun(void)
{
    ...
    {
        p = (EmvaTime *)Link_List_GetAt(g_timeInstancelist, i);
        if (p == NULL)
            return;
...
EmvaTime *timerNew(timeModeU mode, uint8 sub_time_num)
{
    ...
    {
        g_timeInstancelist = Link_List_Init();
        character_dev_event(g_fd, NULL, time_dev_event_callback);
    }
    Link_List_Insert(g_timeInstancelist, p, -1);
    character_dev_ioctl(g_fd, NULL, (uint32)TRUE, NULL);
    return p;
}

static EmvaTime *ThisNew(void) { return timerNew(CYCLE, 1); }
static EmvaTime *ThisSingleNew(void) { return timerNew(SINGLE, 1); }
static EmvaTime *ThisSubNew(uint8 sub_time_num) { return timerNew(CYCLE, sub_time_num); }
static EmvaTime *ThisSubSingleNew(uint8 sub_time_num) { return timerNew(SINGLE, sub_time_num); }
